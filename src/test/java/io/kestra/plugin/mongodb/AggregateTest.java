...
class AggregateTest extends MongoDbContainer {
        ...
        Aggregate aggregate = Aggregate.builder()
            ...
            .pipeline(
                Property.ofValue(
                    Arrays.asList(
                        ImmutableMap.of(
                            "$match", ImmutableMap.of(
                                "status", "available"
                            )
                        ),
                        ImmutableMap.of(
                            "$group", ImmutableMap.of(
                                "_id", "$category",
    ...
    void testProjection() throws Exception {
        ...
        assertThat(
            ((Number) firstBook.get("discountedPrice")).doubleValue(),
